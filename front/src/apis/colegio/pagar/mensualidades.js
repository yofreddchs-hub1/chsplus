...
export default function Mensualidad(props) {
    ...
    const Select_meses = async(valores)=>{
        ...
        if ((abono && Number(abono)>0) || (abonod && Number(abonod)>0)){
            ...
            console.log(total, totald, Periodo)
        ...
        let Fmensualidad = await genera_fromulario({valores:{meses:Rmensualidades}, campos: Form_todos('Form_Mensualidades', Config) })
    ...
    const Meses = async() =>{
        ...
        for (var i=0; i<datos.valores.representados.length; i++){
            ...
            // console.log(val)
            let Formu = Form_todos('Form_Mensualidad', Config)
    ...
    const Inicio = async() =>{
        ...
        let Fcambio = await genera_fromulario({valores:{cambio}, campos: Form_todos('Form_Cambio', Config) })
                    ...
                    {formulario && formulario.Meses
                        ?   formulario.Meses.map(val=>
                                <Grid item container key={val._id} justifyContent="center" alignItems="center">
                                    <Grid item xs={4}> 
                                        <Typography variant="h6" gutterBottom component="div" sx={{...Config ? {color:Config.Estilos.Input_label.color} :{}}}>
                                            {val.label}
                                        </Typography>
                                    </Grid>
