...
export default function Formapago(props) {
    ...
    const Formularios = async(item, props)=>{
        let campos = {...Form_todos('Form_Forma_Pago', Config)};
        ...
        <Box sx={{ textAlign:'left' }}>
            ...
            {state && state.Formas
                ?   <Grid container spacing={1} alignItems="center" justifyContent="center" sx={{marginTop:1}}>
                        <Grid item xs={5}/>
                        <Grid item xs={1}>
                            <IconButton onClick={Agregar} aria-label="add" style={{...Config.Estilos ? Config.Estilos.Botones.Aceptar : {}}}>
                                <Icon color={'#fff'}>add</Icon>
                            </IconButton>
                        </Grid>
                        <Grid item xs={1}>
                            <IconButton onClick={Quitar} disabled={state.Formas.length===1} aria-label="add" style={{...Config.Estilos ? Config.Estilos.Botones.Eliminar : {}}}>
