...
export default class Registros extends Component {
  ...
  Editores = (campo)=>{
    ...
    return <TablaMultiple
                ...
                Columnas={2} 
                Form_origen = {Form_todos(`Form_${nuevo_campo}`, Config)}
                ...
                Titulos_tabla = {Titulos_todos(`Titulos_${nuevo_campo}`, Config)}
            />
  }
  ...
  render(){
    const {Bloques, cargando, Config}=this.state;
    return (
      <div style={{width:'100%', position: "relative"}}>
        <Cuerpo Bloques={Bloques} Config={Config}/>
