...
export default function Inscripcion(props) {
    ...
    const Agregar = async(dato)=>{
        ...
        let festudiante = await genera_fromulario({ valores:{representante}, campos: Form_todos('Form_Estudiante', Config) });
        ...
        festudiante.botones=[
            {
                ...
                onClick: Guardar('colegio_estudiante', Form_todos('Form_Estudiante', Config)), validar:'true', 
    ...
    const Buscar = async()=>{
        ...
        if (representante.Respuesta==='Ok'){
            ...
            let frepresentante = await genera_fromulario({ valores:representante, campos: Form_todos('Form_Representante', Config) })
            let Bloques={
            ...
            if (representante.nombres){
                frepresentante.botones=[
                    {
                      ...
                      variant:"contained", color:"success", icono:<CheckIcon/>,
                      onClick: Guardar('colegio_representante', Form_todos('Form_Representante', Config)), validar:'true', 
                      sx:{...Config.Estilos.Botones ? Config.Estilos.Botones.Aceptar : {}},
                    },
                    {
                ...
                Bloques[`${representante.nombres} ${representante.apellidos}`]= <Formulario {...frepresentante} config={Config}/>
                ...
                for (var i=0; i<representante.representados.length; i++){
                    ...
                    let festudiante = await genera_fromulario({ valores:val, campos: Form_todos('Form_Estudiante', Config) });
                    festudiante.titulos[7].value.estatus.disabled=true;
                    festudiante.botones=[
                        {
                          name:'guardar', label:'Guardar', title:'Guardar datos de estudiante',
    ...
    return (
        <Box sx={{ flexGrow: 1, position:'relative' }}>
            <Grid container spacing={0.5}>
                ...
                <Grid item xs={12}>
                    <Item sx={{height:'79vh', overflowY:'auto'}}>
                        {state.Bloques
