...
import {numeroALetras} from '../../../../procesos/servicios';
...
const Recibopdf = (props) =>{
    ...
    {Formas_pago.map((val, i)=>{
        console.log(val)
        const mensaje = ` > ${val.formapago.titulo} ${val.bancoorigen ? 'Banco Origen: ' + val.bancoorigen.titulo + ',' : ''} ${val.bancodestino ? 'Banco Destino: ' + val.bancodestino.banco.titulo + ',' : ''} ${val.referencia ? 'Referencia: ' + val.referencia + ',' : '' } ${val.formapago.value=== 'efectivodolar' ? '$': 'Bs.'} ${val.monto}`;
